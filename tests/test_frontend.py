...
def test_minimal_cataloguing_view(app):
    app.run()
    assert app.dataframe[0].value.shape == (195, 6)
...
def test_save_match(test_cards, app, tmp_path):
    ...
    app.session_state["cards_df"] = test_cards.copy()
...
# parameterised to check first 6 cards succeed - can change to all cards for a complete test
